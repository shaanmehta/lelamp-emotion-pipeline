...
*Generated by `make eval` on 2026-09-15 19:41:50. Every number here comes out of `artifacts/results.json`.*
...
| stage | p50 (ms) | p95 (ms) |
|---|---|---|
| mp4 decode (8 frames) | 167.7 | 331.5 |
| CLIP, 1 frame | 18.2 | 19.0 |
| CLIP, 8 frames | 63.5 | 64.3 |
| **TIER 1 total (frame → behaviour)** | 19.0 | 19.9 |
| RoBERTa encode | 15.4 | 15.7 |
| temporal pooling | 0.1 | 0.1 |
| 3 fusion heads | 2.2 | 2.3 |
| belief update | 0.0 | 0.0 |
| behaviour policy | 0.1 | 0.1 |
| **TIER 2 total (→ COMMIT_STATE)** | 2.8 | 3.0 |
| **time to first spoken token** | 541.6 | 553.0 |

- **Reflexive budget 300ms: met**, p95 = 20ms, about 15x margin. The lamp moves well inside the window where a motion still reads as a reaction.
- **Verbal budget 800ms to first token: met**, p95 = 553ms.
- **`mp4 decode` is deliberately not inside the reflexive total.** It's an artefact of replaying a dataset: on a robot the camera hands you decoded frames and there's no container to demux. I left it in the table rather than dropping it, because it is a real cost of this harness and hiding it would make the 20ms look cheaper than the measurement was.
- Perception real-time factor p95 = **0.250**, which has to be well under 1 to consume a live stream. Peak RSS **2451MB**.
...
**The body and the voice commit at different times, on purpose.** The state
event can fire halfway through a sentence, as soon as the posterior is confident
enough. The spoken reply doesn't: its budget is defined as 800ms from the end of
the utterance, and generating a reply from the full transcript while the person
is still talking would mean answering words the lamp hasn't heard. In a typical
run the body commits around 50% coverage and the voice follows 0.8 to 3.5
seconds later, at the endpoint. You can see both timestamps in the JSONL:
`t_emit_ms` for the state, `latency.speech_t_ms` for the reply.

**The spoken lines are the weakest part of the system.** Qwen2.5-1.5B at 4-bit
produces a coherent, state-appropriate reply most of the time, and the
counterfactual demo shows the state genuinely changes what it says. But some
lines are clumsy ("I am adjusting to your question") and it occasionally
paraphrases the input back instead of responding to it. There's an echo guard
that catches the worst cases by token overlap and falls back to the template
responder, but it's a threshold and it misses some. The budget has room for a 3B
model, which is the obvious fix and the one I'd make first. I'd rather say this
than have you notice it in the demo.
